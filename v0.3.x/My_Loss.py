...
class HardTripletLoss(nn.Module):
    ...
    def forward(self, attributes, embeddings, labels):
        ...
        #triplet_loss.requires_grad=True
            ...
            #print('-'*100)
        #if triplet_loss == 0:
            #print(hardest_positive_dist - hardest_negative_dist + self.margin)
        return triplet_loss
class HardTripletLoss_D(nn.Module):
    ...
    def forward(self, attributes, embeddings, labels):
        ...
        #print(hardest_positive_dist_index)
        #print(hardest_positive_dist.size())
        #print(hardest_positive_dist_index.size())
        #print(hardest_positive_dist_index[0][1],hardest_positive_dist_index[0][2])
        ...
        #print(hardest_negative_dist_img)
        #print(hardest_negative_dist_img.size())
        ...
        #print(embeddings.size()[1])
        triplet_loss = (torch.sum(triplet_loss_all) / (num_hard_triplets+1e-16)) +  (torch.sum(triplet_loss_all_img) / (num_hard_triplets_img+1e-16))
        #triplet_loss = torch.mean(triplet_loss)
        #triplet_loss.requires_grad=True
...
def _pairwise_distance(bat_attributes, bat_features, bat_lables):
    ...
    #print(bat_attributes.size())
    #print(bat_features.size())
...
def _get_anchor_negative_triplet_mask_feature(relations,labels):
    # Return a 2D mask where mask[a, n] is True iff a and n have distinct labels.
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # Check if labels[i] != labels[k]
    mask_neg = torch.ones_like(relations).to(device).byte()
