...
def main():
    ...
    relation_network = RelationNetwork(2048,400)        #4096输入 2048d+2048d 400隐藏层
    triplet_network = TripletNetwork(relation_network)  #metric learning
...
####################################foward is true###################################################
    ...
    for episode in range(EPISODE):
        ...
        batch_features = Variable(batch_features).cuda(GPU).float()  # 32*1024 视觉特征 Variable应该是要送进图
        sample_features = attribute_network(Variable(sample_attributes).cuda(GPU)) #k*312 映射属性到视觉空间


        sample_features_ext = sample_features.unsqueeze(0).repeat(BATCH_SIZE,1,1) #32*sample.size
        batch_features_ext = batch_features.unsqueeze(0).repeat(class_num,1,1)   #类别数*32*1024
